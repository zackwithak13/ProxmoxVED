...
mkdir -p -m 700 ~/.gnupg
curl -fsSL https://download.onlyoffice.com/GPG-KEY-ONLYOFFICE >/dev/null | gpg --no-default-keyring --keyring gnupg-ring:/tmp/onlyoffice.gpg --import >/dev/null
chmod 644 /tmp/onlyoffice.gpg
chown root:root /tmp/onlyoffice.gpg
mv /tmp/onlyoffice.gpg /usr/share/keyrings/onlyoffice.gpg
echo "deb [signed-by=/usr/share/keyrings/onlyoffice.gpg] https://download.onlyoffice.com/repo/debian squeeze main" >/dev/null | tee /etc/apt/sources.list.d/onlyoffice.list >/dev/null
$STD apt-get update
